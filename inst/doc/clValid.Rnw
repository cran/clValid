...
\SweaveOpts{engine=R,eps=FALSE,echo=TRUE, eval=TRUE}
...
NOTE2: The following example uses package \pkg{mclust}, see the note
below for usage.
...
intern <- clValid(express, 2:6, clMethods=c("hierarchical","kmeans","diana","fanny","som","pam","sota","clara","model"),
                  validation="internal")
@ 
...
<<stability,eval=TRUE>>=
stab <- clValid(express, 2:6, clMethods=c("hierarchical","kmeans","diana","fanny","som","pam","sota","clara","model"),
...
bio <- clValid(express, 2:6, clMethods=c("hierarchical","kmeans","diana","fanny","som","pam","sota","clara","model"),
...
<<BHI,eval=FALSE, echo=FALSE>>=
plot(bio, measure="BHI", legendLoc="topleft")
@ 


<<BSI,eval=FALSE, echo=FALSE>>=
...
if(require("Biobase") && require("annotate") && require("GO") && require("moe430a")) {
  ...
  bio2 <- clValid(express, 2:6, clMethods=c("hierarchical","kmeans","diana","fanny","som","pam","sota","clara","model"),
